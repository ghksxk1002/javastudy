	...
	public void run() {
		try {
			...
			while (true) {
				...
				// SimpleHTTPServer에서는 Header 만 읽음
				if ("".equals(line)) {
					break;
				}
			...
			if ("GET".equals(tokens[0])) {
				consoleLog("requst:" + tokens[1]);
				reponseStaticResource(outputStream, tokens[1], tokens[2]);

			} else {
				System.out.println(tokens[0]);
				// metiods: POST, PUT, DELETE, HEAD, CONNECT
				// SimpleHttpServer 에서는 무시(400 Bad Requst 처리)
				response400Error(outputStream, tokens[1], tokens[2]);

			}
			// 예제 응답입니다.
			// 서버 시작과 테스트를 마친 후, 주석 처리 합니다.
//			outputStream.write("HTTP/1.1 404 Not Found\n".getBytes("UTF-8"));// http 프로트콜에 맞춘 응답
//			outputStream.write("Content-Type:text/html; charset=utf-8\n".getBytes("UTF-8"));
//			outputStream.write("\n".getBytes());// 빈계행 header 부분과 body부분이의 경게를 나타넴
